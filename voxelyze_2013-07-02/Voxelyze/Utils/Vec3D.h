...
//	template <typename U> friend Vec3D<T> operator+(Vec3D<T> const& lhs, Vec3D<U> const& rhs){return Vec3D<T>(lhs.x+rhs.x, lhs.y+rhs.y, lhs.z+rhs.z);}
	...
	const bool operator==(const Vec3D& v) {return (x==v.x && y==v.y && z==v.z);} //Is equal
	const bool operator!=(const Vec3D& v) {return !(*this==v);} //Is not equal
...
//		if (theta < DISCARD_ANGLE_RAD) {*this = CQuat(1,0,0,0); return;} //very small angle, return no rotation
		...
		// Vec3D Axis = RotateFrom.Cross(RotateTo); //Axis of rotation
		Vec3D<> Axis = RotateFrom.Cross(RotateTo); //Axis of rotation (nac: changed from above)
		Axis.NormalizeFast();
...
//	template <typename U> friend Vec3D<T> operator+(Vec3D<T> const& lhs, Vec3D<U> const& rhs){return Vec3D<T>(lhs.x+rhs.x, lhs.y+rhs.y, lhs.z+rhs.z);}
	...
	const bool operator==(const CQuat& s) const {return (w==s.w && x==s.x && y==s.y && z==s.z);} //Is equal
	const bool operator!=(const CQuat& s) const {return (w!=s.w || x!=s.x || y!=s.y || z!=s.z);} //Is equal
...
//		if (squareLength < SLTHRESH_DISCARD_ANGLE){angle=0; axis=Vec3D<T>(1,0,0);} //Not worth doing small angle because requires two sqrts
	...
	void AngleAxis(T *pAngle, T* pAxisX, T* pAxisY, T* pAxisZ) const {T Ang = *pAngle; Vec3D<> Ax /*nac: Vec3D Ax*/ = Vec3D<T>(*pAxisX, *pAxisY, *pAxisZ); AngleAxis(Ang, Ax); *pAngle=Ang; *pAxisX = Ax.x; *pAxisY = Ax.y; *pAxisZ = Ax.z; }
...
//		else return Vec3D(x, y, z)*2.0*acos(w>1?1:w)/sqrt(squareLength);
	...
	const bool IsNegligibleAngle() const {return 2.0*acos(w) < DISCARD_ANGLE_RAD;}
	const bool IsSmallAngle() const {return w>SMALL_ANGLE_W;} 
	const bool IsSmallishAngle() const {return w>SMALLISH_ANGLE_W;}
