...
void CVXC_Structure::DeleteData(void) //sandbox the creation and destruction...
{
	...
	if (muscleTypeArray != NULL){ //clear current array
		delete [] muscleTypeArray;
		muscleTypeArray = NULL;
	}
	if (sensorTypeArray != NULL){ //clear current array
		delete [] sensorTypeArray;
		sensorTypeArray = NULL;
	}
	if (synapseWeightArray != NULL){ //clear current array
		delete [] synapseWeightArray;
		synapseWeightArray = NULL;
	}
	if (nodeBiasArray != NULL){ //clear current array
		delete [] nodeBiasArray;
		nodeBiasArray = NULL;
	}
	if (nodeTimeConstantArray != NULL){ //clear current array
		delete [] nodeTimeConstantArray;
		nodeTimeConstantArray = NULL;
	}
	if (sensorPosition != NULL){ //clear current array
		delete [] sensorPosition;
		sensorPosition = NULL;
	}
	if (hiddenPosition != NULL){ //clear current array
		delete [] hiddenPosition;
		hiddenPosition = NULL;
	}
	if (musclePosition != NULL){ //clear current array
		delete [] musclePosition;
		musclePosition = NULL;
	}
}
...
void CVXC_Structure::IniData(int Size)
{
	...
	if (Size > 0){
		pData = new char[Size];
		muscleTypeArray = new int[Size];
		sensorTypeArray = new int[Size];
		// std::cout << "NumSynapses: " << NumSynapses << std::endl;
		synapseWeightArray = new float[NumSensors*NumHidden+NumHidden*(NumHidden+NumMuscles)];
		nodeBiasArray = new float[NumSensors+NumHidden+NumMuscles];
		nodeTimeConstantArray = new float[NumSensors+NumHidden+NumMuscles];
		sensorPosition = new float[NumSensors*3];
		hiddenPosition = new float[NumHidden*3];
		musclePosition = new float[NumMuscles*3];
		m_SizeOfArray = Size;
		DataInit = true;
	}
}
...
bool CVXC_Structure::ReadXML(CXML_Rip* pXML, std::string Version, std::string* RetMessage)
{
	...
	if (!pXML->FindLoadElement("NumSensors", &NumSensors)) NumSensors = 1;
	if (!pXML->FindLoadElement("NumPacemaker", &NumPacemaker)) NumPacemaker = 1;
	if (!pXML->FindLoadElement("NumMuscles", &NumMuscles)) NumMuscles = 1;
	if (!pXML->FindLoadElement("NumHidden", &NumHidden)) NumHidden = 1;
		
	CreateStructure(X_Voxels, Y_Voxels, Z_Voxels); //create the space for the structure

	if (Version == "" || Version == "0.93")
	{ //newest version
		...
		if (pXML->FindElement("MuscleType"))
		{
			for (int i=0; i<Z_Voxels; i++){
				std::string DataIn;
				std::string RawData;
				...
				//different compression types
				if (Compression == "LONG_READABLE") 
				{
					std::vector<std::string> RawDataVector = split(RawData, ",", false); 
					// DataIn.resize(RawDataVector.size());
					for (int k=0; k<(int)RawDataVector.size(); k++)
					{
						// std::cout << "muscle assignment:" << atoi(RawDataVector[k].c_str()) << std::endl;
						SetMuscleTypeArray(X_Voxels*Y_Voxels*i + k, atoi(RawDataVector[k].c_str()) ); //pDataIn[k];		
					}
					//continue;
				}
			}
		...
		if (pXML->FindElement("SensorType"))
		{
			for (int i=0; i<Z_Voxels; i++){
				std::string DataIn;
				std::string RawData;
				...
				if (Compression == "LONG_READABLE") 
				{
					...
					for (int k=0; k<(int)RawDataVector.size(); k++)
					{
						SetSensorTypeArray(X_Voxels*Y_Voxels*i + k, atoi(RawDataVector[k].c_str()) ); //pDataIn[k];		
					}
					//continue;
				}
			}
		...
		if (pXML->FindElement("SynapseWeight"))
		{
			// for (int i=0; i<NumSensors*NumHidden+NumHidden*(NumMuscles+NumHidden); i++)
			// {
				// time constant
				// synapse weight
				// presynaptic bias
				...
				if (Compression == "LONG_READABLE") 
				{
					...
					for (int k=0; k<(int)RawDataVector.size(); k++)
					{
						SetSynapseWeight(k, atof(RawDataVector[k].c_str()) ); //pDataIn[k];
					}
					//continue;
				}
			// }
		...
		if (pXML->FindElement("NodeBias"))
		{
			...
			// {
				...
				if (Compression == "LONG_READABLE") 
				{
					...
					for (int k=0; k<(int)RawDataVector.size(); k++)
					{
						SetNodeBias(k, atof(RawDataVector[k].c_str()) ); //pDataIn[k];
					}
					//continue;
				}
			// }
		...
		if (pXML->FindElement("NodeTimeConstant"))
		{
			// for (int i=0; i<NumSensors+NumHidden+NumMuscles; i++)
			// {
				// time constant
				...
				if (Compression == "LONG_READABLE") 
				{
					...
					for (int k=0; k<(int)RawDataVector.size(); k++)
					{
						SetNodeTimeConstant(k, atof(RawDataVector[k].c_str()) ); //pDataIn[k];
					}
					//continue;
				}
			// }
		...
		if (pXML->FindElement("SensorPositions"))
		{
			...
			// {
				...
				if (Compression == "LONG_READABLE") 
				{
					...
					for (int k=0; k<(int)RawDataVector.size(); k++)
					{
						SetSensorPosition(k, atof(RawDataVector[k].c_str()) ); //pDataIn[k];
						// std::cout << "sensor array index " << k << " setting sensor " << int(floor(k/3)) << ", index " << int(k % 3) << " to " << atof(RawDataVector[k].c_str()) << std::endl;
					}
					//continue;
				}
			// }
...
void CVXC_Structure::ResetStructure() //erases all voxel imformation within voxel array
{
	...
	{
		SetData(i, 0);
		SetMuscleTypeArray(i, 0);
		SetSensorTypeArray(i, 0);
	}
}
