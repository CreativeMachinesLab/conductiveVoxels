...
{
public:
	CVXC_Structure() {pData = NULL; muscleTypeArray = NULL; sensorTypeArray = NULL; synapseWeightArray = NULL; nodeBiasArray = NULL; nodeTimeConstantArray = NULL; sensorPosition=NULL; hiddenPosition=NULL; musclePosition=NULL; ClearStructure();};
	CVXC_Structure(int xV, int yV, int zV) {pData = NULL; muscleTypeArray = NULL; sensorTypeArray = NULL; synapseWeightArray = NULL; nodeBiasArray = NULL; nodeTimeConstantArray = NULL; sensorPosition=NULL; hiddenPosition=NULL; musclePosition=NULL; CreateStructure(xV, yV, zV);};
	...
	inline int GetMyMuscleTypeArray(int Index) const {if (DataInit) return muscleTypeArray[Index]; else return -1;} //Gets the material index here (this should be the only place we access pData)
	inline int GetMySensorTypeArray(int Index) const {if (DataInit) {return sensorTypeArray[Index];} else return -1;} //Gets the material index here (this should be the only place we access pData)
	// inline int GetSynapseWeight(int Index) const {if (DataInit) return synapseWeightArray[Index]; else return -1;} //Gets the material index here (this should be the only place we access pData)
	inline float GetSynapseWeight(int Index1, int Index2) const  
	{
		if (DataInit) 
		{
			int Index; 
			if(Index1<NumSensors)
			{
				Index = Index1*NumHidden+Index2;
			}
			else if (Index2<NumSensors+NumHidden)
			{
				Index = NumHidden*NumSensors + (Index1-NumSensors)*NumHidden + (Index2-NumSensors);
			}
			else
			{
				Index = NumHidden*NumSensors + NumHidden*NumHidden + (Index1-NumSensors)*(NumMuscles) + (Index2-NumSensors-NumHidden);
			}
			return synapseWeightArray[Index]; //Gets the material index here (this should be the only place we access pData)
		}
		else
		{
			return 0;
	...
	inline float GetNodeBias(int Index) const {if (DataInit) return nodeBiasArray[Index]; else return -1;} //Gets the material index here (this should be the only place we access pData)
	inline float GetNodeTimeConstant(int Index) const {if (DataInit) return nodeTimeConstantArray[Index]; else return -1;} //Gets the material index here (this should be the only place we access pData)
	inline float GetSensorPosition(int Index, int XYZ) const {if (DataInit) return sensorPosition[Index*3+XYZ]; else return -1;} //Gets the material index here (this should be the only place we access pData)
	inline float GetHiddenPosition(int Index, int XYZ) const {if (DataInit) return hiddenPosition[Index*3+XYZ]; else return -1;} //Gets the material index here (this should be the only place we access pData)
	inline float GetMusclePosition(int Index, int XYZ) const {if (DataInit) return musclePosition[Index*3+XYZ]; else return -1;} //Gets the material index here (this should be the only place we access pData)
	...
	inline int GetNumSensors(void) const {return NumSensors;};
	inline int GetNumPacemaker(void) const {return NumPacemaker;};
	inline int GetNumMuscles(void) const {return NumMuscles;};
	inline int GetNumHidden(void) const {return NumHidden;};
	...
	inline void SetMuscleTypeArray(int Index, int Data){if (DataInit) muscleTypeArray[Index] = Data;} //sets the material index here
	inline void SetSensorTypeArray(int Index, int Data){if (DataInit) sensorTypeArray[Index] = Data;} //sets the material index here
	inline void SetNodeBias(int Index, float Data){if (DataInit) nodeBiasArray[Index] = Data;} //sets the material index here
	inline void SetNodeTimeConstant(int Index, float Data){if (DataInit) nodeTimeConstantArray[Index] = Data;} //sets the material index here
	inline void SetSynapseWeight(int Index, float Data){if (DataInit) synapseWeightArray[Index] = Data;} //sets the material index here
	inline void SetSensorPosition(int Index, float Data){if (DataInit) sensorPosition[Index] = Data;} //sets the material index here
	inline void SetHiddenPosition(int Index, float Data){if (DataInit) hiddenPosition[Index] = Data;} //sets the material index here
	inline void SetMusclePosition(int Index, float Data){if (DataInit) musclePosition[Index] = Data;} //sets the material index here
...
protected:
	...
	int* muscleTypeArray;
	int* sensorTypeArray;
	float* synapseWeightArray;
	float* nodeBiasArray;
	float* nodeTimeConstantArray;
	float* sensorPosition;
	float* hiddenPosition;
	float* musclePosition;
...
public:
	...
	inline int GetMuscleTypeArray(int Index) const {return Structure.GetMyMuscleTypeArray(Index);} //Gets the material index here (this should be the only place we access pData)
	inline int GetSensorTypeArray(int Index) const {return Structure.GetMySensorTypeArray(Index);} //Gets the material index here (this should be the only place we access pData)
	inline float GetNodeBias(int Index) const {return Structure.GetNodeBias(Index);} //Gets the material index here (this should be the only place we access pData)
	inline float GetNodeTimeConstant(int Index) const {return Structure.GetNodeTimeConstant(Index);} //Gets the material index here (this should be the only place we access pData)
	inline float GetSensorPosition(int Index, int XYZ) const {return Structure.GetSensorPosition(Index,XYZ);} //Gets the material index here (this should be the only place we access pData)
	inline float GetHiddenPosition(int Index, int XYZ) const {return Structure.GetHiddenPosition(Index,XYZ);} //Gets the material index here (this should be the only place we access pData)
	inline float GetMusclePosition(int Index, int XYZ) const {return Structure.GetMusclePosition(Index,XYZ);} //Gets the material index here (this should be the only place we access pData)
	inline float GetSynapseWeight(int Index1, int Index2) const {return Structure.GetSynapseWeight(Index1, Index2);} //Gets the material index here (this should be the only place we access pData)
	// inline float GetSynapseWeight(int Index1, int Index2) const 
	// {
	// 	int Index; 
	// 	if(Index1<NumSensors)
	...
	// 	else
	// 	{
	// 		Index = NumHidden*NumSensors+(Index1-NumSensors)*(NumHidden+NumMuscles)+Index2;
	// 	}
	// 	return Structure.GetSynapseWeight(Index); //Gets the material index here (this should be the only place we access pData)
	// }
	...
	inline int GetNumSensors(void) const {return Structure.GetNumSensors();} //!< Returns the number of voxels in the Z direction.
	inline int GetNumPacemaker(void) const {return Structure.GetNumPacemaker();} //!< Returns the number of voxels in the Z direction.
	inline int GetNumHidden(void) const {return Structure.GetNumHidden();} //!< Returns the number of voxels in the Z direction.
	inline int GetNumMuscles(void) const {return Structure.GetNumMuscles();} //!< Returns the number of voxels in the Z direction.
...
public:
	...
	// ElectricallyActive:
	void SetNeuralSensorType(int _NeuralSensorType) {NeuralSensorType = _NeuralSensorType;}
	void SetNeuralActivatedMuscleType(int _NeuralActivatedMuscleType) {NeuralActivatedMuscleType = _NeuralActivatedMuscleType;}
