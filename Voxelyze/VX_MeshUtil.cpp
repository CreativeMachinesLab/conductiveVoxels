...
void CVX_MeshUtil::ImportSimWithMesh(CVX_Sim* pSimIn, CMesh* pMeshIn) //links the simulation to an existing surface mesh so it can be deformed correctly
{
...
void CVX_MeshUtil::ImportLinkSim(CVX_Sim* pSimIn)
{
	if (pSimIn != NULL) pSim = pSimIn;
	...
	if (pSim){/*
		...
		for (int i=0; i<(int)CalcVerts.size(); i++){
			...
			for (int j=0; j<(int)CalcVerts[i].ConVoxels.size(); j++){
				...
				ThisPos = CurVox.GetCurPos() + (CurVox.S.Angle*CQuat(TempFact*CalcVerts[i].ConVoxels[j].Off)*CurVox.S.Angle.Conjugate()).ToVec(); //todo:scale!
		...
		if (FacetToSIndex.size() != 0){ //if pulling color for full fact
			for (int i=0; i<(int)DefMesh.Facets.size(); i++){
				DefMesh.Facets[i].FColor = pSim->GetCurVoxColor(FacetToSIndex[i], CurSel);
...
void CVX_MeshUtil::GetCurVLoc(CVertexCalc& VertCalc, Vec3D<>* pLocOut)
{
	...
	for (int j=0; j<(int)VertCalc.ConVoxels.size(); j++){
		CVXS_Voxel& CurVox = pSim->VoxArray[pSim->XtoSIndexMap[VertCalc.ConVoxels[j].XIndex]];

		vfloat ScaleFact = CurVox.GetCurScale().x / CurVox.GetOrigSize().x; //TEMP hack for square, isotropic expansion
	...
	*pLocOut = avgPos/TotWeight; //yes, we're subtracting this out just to add it back.
}
...
void CVX_MeshUtil::GetCurVCol(CVertexCalc& VertCalc, CColor* pColOut, int CurSel)
{
	...
	for (int j=0; j<(int)VertCalc.ConVoxels.size(); j++){
		Tmp = pSim->GetCurVoxColor(pSim->XtoSIndexMap[VertCalc.ConVoxels[j].XIndex], CurSel);
