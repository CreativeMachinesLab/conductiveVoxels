...
void CVXS_Voxel::EulerStep(void)
{
	...
	// 	if ( S.ElectricallyActiveNew and (p_Sim->CurTime - S.RepolarizationStartTime) < p_Sim->pEnv->GetTempBase()/05 )
	// 	{
	...
	// CHECK INFREQUENTLY TO SAVE TIME
	// if (p_Sim->CurTime - GetTimeOfLastMuscleUpdate() > 0.01)
	if (p_Sim->CurTime - GetTimeOfLastMuscleUpdate() > p_Sim->pEnv->GetTempPeriod()/20)
	{
		...
		// if (true)
		{
			...
			if ( (p_Sim->CurTime - S.RepolarizationStartTime) < p_Sim->pEnv->GetTempPeriod()/10)
			{
			...
			else
			{
				...
				if ( (p_Sim->CurTime - S.RepolarizationStartTime) < p_Sim->pEnv->GetTempPeriod()/2)
				{
	...
	// {
		...
		// if (p_Sim->CurTime - GetTimeOfLastMuscleUpdate() > 0.100 )
		// if (true)
		// {
	if ( (p_Sim->CurTime - S.RepolarizationStartTime) < p_Sim->pEnv->GetTempPeriod()/2)
	{
		...
		float newSize = (1 + p_Sim->pEnv->GetTempAmplitude()*GetCTE() * sin( 2*3.14* (p_Sim->CurTime - S.RepolarizationStartTime)/(p_Sim->pEnv->GetTempPeriod()/2) ) )   *OriginalSize.x;
...
float CVXS_Voxel::UpdateElectricalActivationFromNeighbors(void)
{
	...
	for (int i=0; i<NumLocBond; i++){
		// std::cout << "checking bond " << i << std::endl;
		...
		if ( pThisNeighborVoxel->GetElectricallyActiveOld() )  
		{ 
			// std::cout << "repolarizing at " << p_Sim->CurTime << std::endl;
			// if ( _pMat->GetIsConductive() )
			// {
			// 	S.ElectricallyActiveNew = true; 
			// }
