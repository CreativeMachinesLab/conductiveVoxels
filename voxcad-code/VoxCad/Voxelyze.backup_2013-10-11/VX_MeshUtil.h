...
struct CVertexComp { //a component of a inter-cube vertex average
	CVertexComp() {}
	CVertexComp(int XIndexIn, Vec3D<> OffIn, vfloat WeightIn) {XIndex = XIndexIn; Off = OffIn;; Weight = WeightIn;}
	inline CVertexComp& operator=(const CVertexComp& V) {XIndex=V.XIndex; Off=V.Off; Weight=V.Weight; return *this;}; //overload equals
...
public:
	...
	void ImportSimWithMesh(CVX_Sim* pSimIn, CMesh* pMeshIn); //links the simulation to an existing surface mesh so it can be deformed correctly
	void ImportLinkSim(CVX_Sim* pSimIn); //creates a surface mesh to look like the voxels... (for rectangualr voxels...)
	CVX_Sim* pSim;
