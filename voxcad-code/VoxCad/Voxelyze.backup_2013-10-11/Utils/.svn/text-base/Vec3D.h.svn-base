...
//		if (squareLength < SLTHRESH_DISCARD_ANGLE){angle=0; axis=Vec3D<T>(1,0,0);} //Not worth doing small angle because requires two sqrts
	...
	void AngleAxis(T *pAngle, T* pAxisX, T* pAxisY, T* pAxisZ) const {T Ang = *pAngle; Vec3D Ax = Vec3D(*pAxisX, *pAxisY, *pAxisZ); AngleAxis(Ang, Ax); *pAngle=Ang; *pAxisX = Ax.x; *pAxisY = Ax.y; *pAxisZ = Ax.z; }
