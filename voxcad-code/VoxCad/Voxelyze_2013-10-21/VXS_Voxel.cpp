...
void CVXS_Voxel::EulerStep(void)
{
	...
	{	
		if (p_Sim->GetNeuralOutputOfIndex( /*_pMat->GetNeuralActivatedMuscleType()*/GetMuscleType()-1) > 0.0 ) //p_Sim->pEnv->GetActivationThreshold() )  // nac: add this as a parameter?
		{
	...
	else
	{
		if ( /*_pMat->GetNeuralSensorType()*/GetSensorType() )
		{
			...
			{
				p_Sim->SetSensorValueOfIndex( /*_pMat->GetNeuralSensorType()*/ GetSensorType() -1, 0.0);
			}
		}
	}
